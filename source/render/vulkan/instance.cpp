...
namespace hs::ren::vk {
    VkBool32 vk::Instance::debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT msg_severity,
        ...
        check_layer(msg_severity, msg_data->pMessage);
        return VK_FALSE;
    }

    Instance::Instance(Hid &hid) {
        uint32_t extension_count = 0;
        check(vkEnumerateInstanceExtensionProperties(nullptr, &extension_count, nullptr), "vkEnumerateInstanceExtensionProperties");
        this->available_instance_extensions = std::vector<VkExtensionProperties>(extension_count);
        check(vkEnumerateInstanceExtensionProperties(nullptr, &extension_count, this->available_instance_extensions.data()), "vkEnumerateInstanceExtensionProperties");

        uint32_t layer_count = 0;
        check(vkEnumerateInstanceLayerProperties(&layer_count, nullptr), "vkEnumerateInstanceLayerProperties");
        this->available_instance_layers =  std::vector<VkLayerProperties>(layer_count);
        check(vkEnumerateInstanceLayerProperties(&layer_count, this->available_instance_layers.data()), "vkEnumerateInstanceLayerProperties");
        ...
        info("[VK] All required instance extensions supported");
        ...
        info("All required instance layers supported");
        ...
        check(vkCreateInstance(&instanceCreateInfo, nullptr, &this->instance), "vkCreateInstance");
        ...
        check(vkCreateDebugUtilsMessengerEXT(this->instance, &debugCreateInfo, nullptr, &this->debugMessenger), "vkCreateDebugUtilsMessengerEXT");
    }
